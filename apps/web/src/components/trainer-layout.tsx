...
import { Home, LogOut, PanelLeftClose, PanelLeftOpen, UserRound, Users } from 'lucide-react';
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (current.role !== 'TRAINER') return router.replace('/athlete/training');
    ...
    <div className="min-h-screen bg-background text-foreground">
      <aside className={`fixed inset-y-0 left-0 z-40 hidden border-r border-border bg-sidebar/95 p-4 backdrop-blur-xl transition-all duration-300 lg:flex lg:flex-col ${collapsed ? 'w-24' : 'w-72'}`}>
        <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
          <Link className="rounded-2xl border border-primary/20 bg-black/30 px-2 py-1 shadow-glow hover:bg-primary/5" href="/trainer/feed">
            <BrandLogo size={collapsed ? 'compact' : 'sm'} />
          </Link>
          {!collapsed && (
            <button aria-label="Recolher menu" className="btn-ghost h-10 w-10 p-0" onClick={() => setCollapsed(true)} type="button">
        ...
        {collapsed && (
          <button aria-label="Expandir menu" className="btn-ghost mx-auto mt-4 h-10 w-10 p-0" onClick={() => setCollapsed(false)} type="button">
            <PanelLeftOpen size={18} />
          </button>
        )}

        {!collapsed && <p className="mt-4 px-2 text-xs font-medium text-muted">Coach performance suite</p>}
        ...
        <nav className="mt-8 space-y-2">
          {navigation.map((item) => {
            ...
            return (
              <Link
                className={`group relative flex h-12 items-center gap-3 rounded-2xl px-3 text-sm font-semibold ${active ? 'bg-primary/10 text-primary' : 'text-muted hover:bg-surface-hover hover:text-foreground'} ${collapsed ? 'justify-center' : ''}`}
                href={item.href}
                key={item.href}
      ...
      <header className="fixed inset-x-0 top-0 z-40 border-b border-border bg-sidebar/90 backdrop-blur-xl lg:hidden">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          <Link className="rounded-2xl border border-primary/20 bg-black/30 px-2 shadow-glow" href="/trainer/feed">
